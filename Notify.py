...
def heartbeat_embed(snapshot):
    """
    Daily portfolio + cluster snapshot. snapshot: {
      "healthy": True, "checks_today": 47, "last_run_utc": "08:58",
      "active_clusters": [{"ticker":"NVDA","count":3},{"ticker":"LMT","count":2}],
      "holdings": {"congress": 38, "insiders": 61},   # positions per group
      "top_held": ["NVDA","MSFT","LMT"],
      "trades_24h": 7,
    }
    """
    ...
    cluster_str = "  ·  ".join(
        f"${c.get('ticker','?')} ×{c.get('count','?')}" for c in clusters
    ) or "none active"
    ...
    for grp, cnt in holdings.items():
        fields.append(_field(grp.capitalize(), f"{cnt} positions"))
    fields.append(_field("Top held", " · ".join(snapshot.get("top_held") or []) or "—"))
    fields.append(_field("Last 24h", f"+{snapshot.get('trades_24h', 0)} trades"))
...
if __name__ == "__main__":
    ...
    send(url, [
        ...
        heartbeat_embed({
            "healthy": True, "checks_today": 47, "last_run_utc": "08:58",
            "active_clusters": [{"ticker": "NVDA", "count": 3},
                                {"ticker": "LMT", "count": 2}],
            "holdings": {"congress": 38, "insiders": 61},
            "top_held": ["NVDA", "MSFT", "LMT"], "trades_24h": 7,
        }),
    ])
    print("sent.")